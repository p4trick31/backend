...
class SystemReportAdmin(admin.ModelAdmin):
    list_display = ('id', 'user', 'reason', 'short_message', 'created_at', 'is_respond')
...
class DailyReportAdmin(admin.ModelAdmin):
    list_display = ("date", "generated_by", "generated_at", "short_text")
    search_fields = ("text", "generated_by__username")
    list_filter = ("date", "generated_by")
    ordering = ("-date",)

    def short_text(self, obj):
        return obj.text[:75] + "..." if len(obj.text) > 75 else obj.text
    short_text.short_description = "Report Summary"
